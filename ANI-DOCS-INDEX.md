...
Sprint 13 计划：真实 provider / live gate 计划见 repo/development-records/sprint13-real-provider-readiness-plan.md；S01 网络路由 Kube-OVN 已通过 `validate_kubeovn_network_live_gate.py --live --cleanup` 并归档 evidence；S02 K8s workloads vCluster 已通过 `validate_vcluster_live_gate.py --live --proxy-server ... --chart-version 0.34.1` 并归档 workload list evidence；S03 storage Rook-Ceph 已通过 `validate_storage_live_gate.py --live` 并归档 snapshot/mount-target evidence，A 轨 contract 记录为 SPRINT13-STORAGE-ROOK-CEPH-A-TRACK / validate-storage-live-gate；S04 GPU 已完成 B 轨只读盘点与 `GPU_INVENTORY_PROVIDER=kubernetes_rest` 接线补齐，NVIDIA device-plugin `v0.19.2` 真实可见但 DCGM exporter 未发现，仍为 LIVE PENDING / BLOCKED，SPRINT13-GPU-INVENTORY-DCGM-A-TRACK / validate-gpu-inventory-live-gate 固定 NVIDIA device-plugin / DCGM GPU inventory 与 occupancy contract 门禁；SPRINT13-OBJECTSTORE-MINIO-A-TRACK / validate-object-store-live-gate 固定 MinIO bucket 与 upload/download pre-signed URL contract 门禁；SPRINT13-VECTOR-MILVUS-A-TRACK / validate-vector-store-live-gate 固定 Milvus vector document insert 与 search readiness contract 门禁；SPRINT13-INSTANCE-OBSERVABILITY-PROMETHEUS-A-TRACK / validate-instance-observability-live-gate 固定 Prometheus + kubelet / K8s API instance observability logs/events/metrics/security-events/exec contract 门禁；S04-S07 未跑通 live gate 前不得标 runtime/production ready，S04 当前为 LIVE PENDING / BLOCKED，S05-S07 为 LIVE PENDING
...
Sprint 6-10 完成 contract/local/release-prep scaffold（installer、offline、CLI、RC readiness 均为 contract/local validation，非真实发布）；Sprint 11 为首次真实物理服务器验证阶段，包含 Rook-Ceph VM 优先块存储 live 部署，现已转为历史回归门禁；Sprint 12 已完成 Core「Services 支撑 Handler」19 个 handler + 2 个 422 的 Tier1 local profile 收口。历史 Sprint 4 回归门禁（`SPEC-SPLIT-A`、`SPEC-CORE-BETA`、`SPEC-COMPAT-A`、`MOCK-A`、`DOC-API-A`、`SDK-BETA-*`、`SDK-MOCK-SMOKE-*`、`SPRINT4-CLOSURE-A`）有效。详细技术边界与验收命令见 [`repo/CURRENT-SPRINT.md`](repo/CURRENT-SPRINT.md)，已完成批次见 [`repo/development-records/README.md`](repo/development-records/README.md)。当前活跃冲刺为 **Sprint 13 / Core real provider 与 live gate 收敛**：S01 网络路由 Kube-OVN、S02 K8s workloads vCluster 与 S03 storage Rook-Ceph 已通过真实 live gate 并归档 evidence；S04 GPU NVIDIA device-plugin/DCGM 已完成 B 轨只读盘点与 provider 接线补齐，但 DCGM exporter metrics 后端未发现，仍为 LIVE PENDING / BLOCKED；S05 object-store MinIO、S06 vector Milvus 与 S07 instance observability Prometheus 已完成 A 轨 code+contract ready / LIVE PENDING；从 Sprint 12 已闭合的 handler、ports、local adapters 接入真实组件和 evidence JSON 门禁；未跑通对应 live gate 前，对应能力仍只可标 Tier1 local profile，不代表 real-provider/runtime/production ready。
