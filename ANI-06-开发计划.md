...
当前执行入口：Sprint 13 真实 provider / live gate 计划见 repo/development-records/sprint13-real-provider-readiness-plan.md；S01 网络路由 Kube-OVN、S02 K8s workloads vCluster 与 S03 storage Rook-Ceph 已完成 A 轨 code+contract ready（LIVE PENDING），执行时必须先声明真实组件与版本、live gate 命令、evidence 输出路径和失败边界
