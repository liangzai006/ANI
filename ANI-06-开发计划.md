...
> 最后更新：2026-06-20（Sprint 12 / Core「Services 支撑 Handler」已完成 A/B1/B2/B3 全部 19 个 Core handler + 2 个 422 的 Tier1 local profile 收口；当前进入 Sprint 13 / Core real provider 与 live gate 收敛；S01 网络路由 Kube-OVN、S02 K8s workloads vCluster 与 S03 storage Rook-Ceph 已通过真实 live gate 并归档 evidence；S04 GPU 已完成 B 轨只读盘点与 provider 接线补齐，但 DCGM exporter metrics 后端未发现，仍为 LIVE PENDING / BLOCKED；S05-S07 已完成 A 轨 code+contract ready / LIVE PENDING；本仓库只推进 ANI Core，不开发 Services/RAG/Console/BOSS/frontends；Sprint 11 / Core Real Deployment Validation 与 Rook-Ceph 正式部署结果继续作为历史回归边界保留；Sprint 13 未跑通对应 live gate 前，不得把对应 local profile 能力标记为 real-provider、runtime ready 或 production ready）
...
当前重心：Sprint 13 / Core real provider 与 live gate 收敛（2026-06-19 起）；Sprint 12 已完成 A/B1/B2/B3 全部 19 个已声明未实现 Core handler + 2 个 422 的 Tier1 local profile，并完成文档闭环；Sprint 13 必须从这些既有代码边界接入真实 provider 与固定 live gate：B1 边界为 `pkg/ports/instance_observability.go`、`pkg/ports/gpu_inventory.go`、`pkg/ports/sandbox_template_catalog.go`、`pkg/adapters/runtime/local_*` 与 Gateway handler；S04 GPU 已补齐 `GPU_INVENTORY_PROVIDER=kubernetes_rest` 接线路径并完成 B 轨只读盘点，但 DCGM exporter 未发现，仍为 LIVE PENDING / BLOCKED；B2 边界为 `pkg/ports/network_resources.go`、`pkg/ports/storage_resources.go`、`pkg/ports/k8s_clusters.go`、`pkg/adapters/runtime/local_*` 与 Gateway handler，S01 网络路由 Kube-OVN 已完成 Core route provider 与 Gateway runtime 接线并通过真实 route live gate，S02 K8s workloads vCluster 已通过真实 workload list live gate，S03 storage Rook-Ceph 已通过真实 snapshot/mount-target live gate；B3 边界为 `pkg/ports/storage_resources.go`、`pkg/ports/object_store.go`、`pkg/ports/vector_store.go`、`pkg/adapters/runtime/storage_service.go`、`pkg/adapters/runtime/vector_store_service.go` 与 Gateway handler；未执行对应 live gate 前，不代表 real-provider、runtime ready 或 production ready；详见 repo/CURRENT-SPRINT.md
当前执行入口：Sprint 13 真实 provider / live gate 计划见 repo/development-records/sprint13-real-provider-readiness-plan.md；S01 网络路由 Kube-OVN 已通过 B 轨真实 live gate（evidence：repo/development-records/live-evidence/sprint13-netroute-kubeovn-live-evidence.json；result：repo/development-records/sprint13-netroute-kubeovn-live-result.md），S02 K8s workloads vCluster 已通过 B 轨真实 workload list live gate（evidence：repo/development-records/live-evidence/sprint13-k8s-workloads-vcluster-live-evidence.json；result：repo/development-records/sprint13-k8s-workloads-vcluster-live-result.md），S03 storage Rook-Ceph 已通过 B 轨真实 snapshot/mount-target live gate（evidence：repo/development-records/live-evidence/sprint13-storage-rook-ceph-live-evidence.json；result：repo/development-records/sprint13-storage-rook-ceph-live-result.md），S04 GPU NVIDIA device-plugin/DCGM 已完成 B 轨只读盘点与 provider 接线补齐（read-only evidence：repo/development-records/live-evidence/sprint13-gpu-inventory-dcgm-readonly-evidence.json；result：repo/development-records/sprint13-gpu-inventory-dcgm-live-result.md），但 DCGM exporter 未发现，仍为 LIVE PENDING / BLOCKED；S05 object-store MinIO、S06 vector Milvus 与 S07 instance observability Prometheus 已完成 A 轨 code+contract ready（LIVE PENDING），B 轨执行时必须先声明真实组件与版本、live gate 命令、evidence 输出路径和失败边界
最近里程碑（详情见 repo/CURRENT-SPRINT.md 已完成切片）：
  2026-06-20  SPRINT13-GPU-INVENTORY-DCGM-B-TRACK-READONLY：S04 GPU B 轨只读盘点与 provider 接线补齐完成；确认 Kubernetes `v1.36.1`、NVIDIA device-plugin `v0.19.2` DaemonSet `3/3 ready`、三节点合计 6 GPU；Gateway/Bootstrap 新增 `GPU_INVENTORY_PROVIDER=kubernetes_rest`，`validate-gpu-inventory-live-gate --live` 支持 Core real dev_profile + DCGM metrics 校验；因未发现 DCGM exporter，S04 仍为 LIVE PENDING / BLOCKED，不代表 real-provider passed [B-track readonly]
...
| Sprint 13 ⭐ | 进行中 | 2026-06-19 起 | 真实 provider / live gate 收敛；从 Sprint 12 已建立的 ports/adapters/router 边界接入 K8s/kubelet/Prometheus/DCGM/Kata/Kube-OVN/Rook-Ceph/MinIO/向量后端等真实组件；S01 网络路由 Kube-OVN、S02 K8s workloads vCluster 与 S03 storage Rook-Ceph 已通过真实 live gate 并归档 evidence；S04 GPU NVIDIA device-plugin/DCGM、S05 object-store MinIO、S06 vector Milvus 与 S07 instance observability Prometheus 已到 code+contract ready / LIVE PENDING（`SPRINT13-INSTANCE-OBSERVABILITY-PROMETHEUS-A-TRACK` / `validate-instance-observability-live-gate` 固定 Prometheus + kubelet / K8s API logs/events/metrics/security-events/exec contract）；计划见 `repo/development-records/sprint13-real-provider-readiness-plan.md`，未执行对应 live gate 前不得标 runtime/production ready |
