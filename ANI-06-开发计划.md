...
> 最后更新：2026-06-21（Sprint 12 / Core「Services 支撑 Handler」已完成 A/B1/B2/B3 全部 19 个 Core handler + 2 个 422 的 Tier1 local profile 收口；当前进入 Sprint 13 / Core real provider 与 live gate 收敛；S01 网络路由 Kube-OVN、S02 K8s workloads vCluster、S03 storage Rook-Ceph、S04 GPU NVIDIA device-plugin/DCGM、S05 object-store MinIO pre-signed URL、S06 vector Milvus 与 S07 instance observability Prometheus + kubelet / K8s API 已通过 production-shaped live gate 并归档 `production_shape.status=passed` evidence；S01 强制经 ANI Gateway create/list 后再观测 Kube-OVN 底层对象，S02-S04 分别固定 metadata target TLS、in-cluster RBAC、cluster Service metrics proof_items 与 workload/storage/GPU+DCGM 正向业务证据，S05 固定 MinIO bucket create/list、upload/download pre-signed URL、实际 PUT/GET 与 cleanup evidence，S06 固定 Milvus vector store create、documents insert、search readiness 与 cleanup evidence，S07 固定 Prometheus readiness、Core logs/events/metrics/security-events/exec evidence 与 cleanup evidence；SPRINT13-AUTH-DEX-PRODUCTION-GATE（Auth/Dex production gate）已通过真实集群生产形态验证，production-shaped Gateway 使用 `ANI_AUTH_MODE=auth_service`，并经 `validate-auth-dex-production-gate` 固定 anonymous 401、OIDC begin/complete 200、protected API bearer 200 与 refresh 200；S01-S07 的 Auth/Dex production ready 阻断已解除，但该结论仍不是 full platform production ready；历史 LIVE PENDING token 仅作门禁兼容语境；本仓库只推进 ANI Core，不开发 Services/RAG/Console/BOSS/frontends；Sprint 11 / Core Real Deployment Validation 与 Rook-Ceph 正式部署结果继续作为历史回归边界保留；S01-S07 production-shaped acceptance passed 不等于 full platform production ready）
...
最近里程碑（详情见 repo/CURRENT-SPRINT.md 已完成切片）：
  ...
  2026-06-21  SPRINT13-INSTANCE-OBSERVABILITY-PROMETHEUS-LIVE-A：S07 instance observability B 轨真实 live gate 通过；production-shaped Gateway 使用 `INSTANCE_OBSERVABILITY_PROVIDER=prometheus_kubernetes` 接入 Prometheus + Kubernetes API/kubelet backend，`validate-instance-observability-live-gate --live --production-shaped --cleanup` 完成 Prometheus readiness、Core logs/events/metrics/security-events/exec session 与临时 Pod/Event cleanup；evidence JSON 已归档；S07 可标 production-shaped acceptance passed，不代表 full platform production ready 或长期 Prometheus/observability HA 规划完成 [B-track live]
  2026-06-20  SPRINT13-OBJECTSTORE-MINIO-LIVE-A：S05 object-store MinIO B 轨真实 live gate 通过；production-shaped Gateway 使用 `OBJECT_STORE_PROVIDER=minio` 接入 MinIO/S3-compatible backend，`validate-object-store-live-gate --live --production-shaped --cleanup` 完成 bucket create/list、upload/download pre-signed URL、实际 PUT/GET、临时 object cleanup 与临时 API key revoke；evidence JSON 已归档；S05 可标 production-shaped acceptance passed，不代表 full platform production ready 或长期 MinIO HA/存储规划完成 [B-track live]
  2026-06-21  SPRINT13-VECTOR-MILVUS-LIVE-A：S06 vector Milvus B 轨真实 live gate 通过；production-shaped Gateway 使用 `VECTOR_STORE_PROVIDER=milvus` 接入 Milvus REST backend，`validate-vector-store-live-gate --live --production-shaped --cleanup` 完成 Milvus readiness、vector store create、documents insert、search readiness、临时 vector store cleanup 与临时 API key revoke；evidence JSON 已归档；S06 可标 production-shaped acceptance passed，不代表 full platform production ready 或长期 Milvus HA/持久化规划完成 [B-track live]
...
| Sprint 13 ⭐ | 进行中 | 2026-06-19 起 | 真实 provider / live gate 收敛；从 Sprint 12 已建立的 ports/adapters/router 边界接入 K8s/kubelet/Prometheus/DCGM/Kata/Kube-OVN/Rook-Ceph/MinIO/向量后端等真实组件；S01 网络路由 Kube-OVN、S02 K8s workloads vCluster、S03 storage Rook-Ceph、S04 GPU NVIDIA device-plugin/DCGM、S05 object-store MinIO、S06 vector Milvus 与 S07 instance observability Prometheus 已通过 production-shaped live gate 并归档 `production_shape.status=passed` evidence；`SPRINT13-INSTANCE-OBSERVABILITY-PROMETHEUS-A-TRACK` / `validate-instance-observability-live-gate` 固定 Prometheus + kubelet / K8s API logs/events/metrics/security-events/exec contract；历史 LIVE PENDING token 仅作门禁兼容语境；计划见 `repo/development-records/sprint13-real-provider-readiness-plan.md`；production-shaped acceptance passed 不等于 full platform production ready |
