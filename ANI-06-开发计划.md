...
> 最后更新：2026-06-19（Sprint 12 / Core「Services 支撑 Handler」已完成 A/B1/B2/B3 全部 19 个 Core handler + 2 个 422 的 Tier1 local profile 收口；当前进入 Sprint 13 / Core real provider 与 live gate 收敛；S01-S06 已完成 A 轨 code+contract ready / LIVE PENDING；本仓库只推进 ANI Core，不开发 Services/RAG/Console/BOSS/frontends；Sprint 11 / Core Real Deployment Validation 与 Rook-Ceph 正式部署结果继续作为历史回归边界保留；Sprint 13 未跑通 live gate 前，不得把 Sprint 12 local profile 能力标记为 real-provider、runtime ready 或 production ready）
...
当前执行入口：Sprint 13 真实 provider / live gate 计划见 repo/development-records/sprint13-real-provider-readiness-plan.md；S01 网络路由 Kube-OVN、S02 K8s workloads vCluster、S03 storage Rook-Ceph、S04 GPU NVIDIA device-plugin/DCGM、S05 object-store MinIO 与 S06 vector Milvus 已完成 A 轨 code+contract ready（LIVE PENDING），执行时必须先声明真实组件与版本、live gate 命令、evidence 输出路径和失败边界
...
| Sprint 13 ⭐ | 进行中 | 2026-06-19 起 | 真实 provider / live gate 收敛；从 Sprint 12 已建立的 ports/adapters/router 边界接入 K8s/kubelet/Prometheus/DCGM/Kata/Kube-OVN/Rook-Ceph/MinIO/向量后端等真实组件；S01 网络路由 Kube-OVN、S02 K8s workloads vCluster、S03 storage Rook-Ceph、S04 GPU NVIDIA device-plugin/DCGM、S05 object-store MinIO 与 S06 vector Milvus 已到 code+contract ready / LIVE PENDING（`SPRINT13-VECTOR-MILVUS-A-TRACK` / `validate-vector-store-live-gate` 固定 Milvus document insert 与 search readiness contract）；计划见 `repo/development-records/sprint13-real-provider-readiness-plan.md`，未执行 live gate 前不得标 runtime/production ready |
