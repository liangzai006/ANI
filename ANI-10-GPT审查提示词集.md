...
> 最后校准：2026-05-19（标注提示词模板边界）

> ⚠️ 本文件是**审查提示词模板**，不是当前开发计划或交付范围的真实来源。
> 使用前必须先读取 `ANI-DOCS-INDEX.md`、`ANI-06-开发计划.md` 和 `repo/CURRENT-SPRINT.md`，并用最新项目状态替换提示词中的示例背景。
...
- 每组提示词中标注了「此处粘贴 XXX 全文」，发送前将对应 ANI 文档内容粘贴进去；不得直接使用本文内的历史示例作为当前事实
...
【5 个月内计划完成的功能清单】
请以 `ANI-06-开发计划.md` Section 零、Sprint 计划和 `repo/CURRENT-SPRINT.md` 为准，不要使用旧的 M1/M2/M3 月度粗排。

【同期还需要完成（ANI-07 部署工程）】
- ani-installer 全流程 TUI 向导（11步，支持裸机/VM/已有K8s）
- 硬件自动检测 + NVIDIA/昇腾/海光驱动安装
- vCluster 租户 K8s 集群能力（v1.0.0）；Karmada 多集群为 Phase 2 预留
- 计量服务（TimescaleDB + 采集Pipeline）
- Patch 升级 Operator + BOSS 升级界面（Phase 2 预留，不作为 v1.0.0 阻塞门禁）
- BOSS 白牌化（Phase 2 预留，不作为 v1.0.0 阻塞门禁）
- 内部 CA + 纯IP部署方案
...
【已设计的安全措施】
对 AI 原生应用的安全设计：
- 多层沙箱：Phase 1 = Kata QEMU + 容器安全护栏；Phase 2+ = Kata Firecracker / gVisor 增强
