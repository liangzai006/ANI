...
2. 已完成 S01 网络路由 Kube-OVN、S02 K8s workloads vCluster 与 S03 storage Rook-Ceph B 轨真实 live gate：S01 route provider 接线、Gateway runtime 注入、真实 create/observe、evidence JSON 与 cleanup 核验均已完成；S02 vCluster CLI 恢复、chart version pin、临时 Deployment create、Core proxy/workload list observe 与 cleanup 已完成；S03 CSI snapshot CRDs/controller 与默认 RBD `VolumeSnapshotClass` 已恢复，Core volume create、snapshot create/list、filesystem create、mount-target list 与 cleanup 已完成。S04 GPU NVIDIA device-plugin/DCGM 已进入 B 轨并完成只读盘点：device-plugin DaemonSet `3/3 ready`、三节点合计 6 GPU，Gateway/Bootstrap `GPU_INVENTORY_PROVIDER=kubernetes_rest` 与 `validate-gpu-inventory-live-gate --live` 已补齐；但 DCGM exporter 未发现，S04 不得标 real-provider passed，下一步需人工确认部署/恢复 DCGM 或提供 metrics endpoint。S05 object-store MinIO、S06 vector Milvus 与 S07 实例观测 Prometheus + kubelet / K8s API A 轨已完成，仍保持 LIVE PENDING，下一步只能在人工确认后进入真实 live gate。
...
| GPU 清单/占用 | NVIDIA device-plugin / DCGM / node labels | `ports.GPUInventory`，复用 Sprint 5 GPU evidence 作为前置事实 | **LIVE PENDING / BLOCKED on DCGM exporter evidence**（A 轨 `SPRINT13-GPU-INVENTORY-DCGM-A-TRACK`；B 轨只读盘点 `sprint13-gpu-inventory-dcgm-live-result.md`；readiness：`sprint13-gpu-inventory-dcgm-readiness.md`；gate：`validate-gpu-inventory-live-gate`；Gateway/Bootstrap `GPU_INVENTORY_PROVIDER=kubernetes_rest` 已补齐；device-plugin 真实可见但 DCGM exporter 未发现，不标 real-provider passed） |
| Sandbox templates | Kata / runtimeClass / template catalog | `ports.SandboxTemplateCatalog` | 待拆分执行 |
| 网络路由 | Kube-OVN | `ports.NetworkService` / `runtime.NetworkService` / `network_resources.go` / `pkg/bootstrap/deps.go` / Gateway network runtime | **real-provider evidence passed for S01 route gate**（Core route provider 与 Gateway runtime 注入完成；`validate_kubeovn_network_live_gate.py --live --cleanup` 已通过；evidence：`development-records/live-evidence/sprint13-netroute-kubeovn-live-evidence.json`；result：`sprint13-netroute-kubeovn-live-result.md`；不代表 production ready） |
| 卷快照与 mount-targets | Rook-Ceph RBD / CSI snapshot / NFS 或等价 filesystem backend | `ports.StorageService` / `runtime.LocalStorageService` / `storage_resources.go` / `storage_runtime.go` | **real-provider evidence passed for S03 storage gate**（A 轨 `SPRINT13-STORAGE-ROOK-CEPH-A-TRACK`；gate：`validate-storage-live-gate`；live result：`sprint13-storage-rook-ceph-live-result.md`；evidence：`development-records/live-evidence/sprint13-storage-rook-ceph-live-evidence.json`；不代表 production ready） |
| K8s workloads | vCluster / Kubernetes API | `ports.K8sClusterService` / `local_k8s_cluster_service.go` / `k8s_cluster_resources.go` | **real-provider evidence passed for S02 workload list gate**（`sprint13-k8s-workloads-vcluster-live-result.md`；evidence：`development-records/live-evidence/sprint13-k8s-workloads-vcluster-live-evidence.json`；不代表 production ready） |
