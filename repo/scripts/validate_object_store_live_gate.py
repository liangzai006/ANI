...
def validate_contract(document: dict[str, Any]) -> None:
    ...
    if not isinstance(tools, list) or not REQUIRED_TOOLS.issubset(set(tools)):
        fail("required_tools must include curl and mc")
...
def main() -> int:
    ...
    validate_docs()
    if args.live:
        fail("--live is human-gated for SPRINT13-OBJECTSTORE-MINIO-A and is not automated in A-track")
    print("SPRINT13-OBJECTSTORE-MINIO-A contract valid; live execution is human-gated")
    return 0
