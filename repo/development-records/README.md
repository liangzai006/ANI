...
| SPRINT13-GPU-INVENTORY-DCGM-B-TRACK-READONLY | S04 GPU inventory / occupancy B 轨只读盘点与 gate 接线补齐：确认 Kubernetes `v1.36.1`、NVIDIA device-plugin `v0.19.2` DaemonSet `3/3 ready`、三节点合计 6 GPU；Gateway/Bootstrap 支持 `GPU_INVENTORY_PROVIDER=kubernetes_rest`，`validate-gpu-inventory-live-gate --live` 支持 Core real dev_profile + DCGM metrics 校验；因未发现 DCGM exporter，S04 仍为 LIVE PENDING / BLOCKED，不标 real-provider passed | sprint13-gpu-inventory-dcgm-live-result.md |
