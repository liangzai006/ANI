...
| # | 切片 | 真实组件（已选） | status |
|---|---|---|---|
| S01 | 网络路由 | Kube-OVN v1.15.8 | production-shaped gate passed（Gateway `POST/GET /networks/routes` create/list + in-cluster ServiceAccount/RBAC + Kube-OVN bottom observation；evidence：`live-evidence/sprint13-netroute-kubeovn-live-evidence.json`；production guard：`validate-sprint13-b-track-production-shape`；不代表 full platform production ready） |
| S02 | K8s workloads | vCluster / K8s v1.36.1 | production-shaped gate passed（Gateway provider create vCluster + metadata target TLS proxy/workload list；evidence：`live-evidence/sprint13-k8s-workloads-vcluster-live-evidence.json`；production guard：`validate-sprint13-b-track-production-shape`；不代表 full platform production ready） |
| S03 | 卷快照 / mount-targets | Rook-Ceph（CSI snapshot / RBD / NFS） | production-shaped gate passed（production gateway + in-cluster RBAC storage lifecycle；evidence：`live-evidence/sprint13-storage-rook-ceph-live-evidence.json`；production guard：`validate-sprint13-b-track-production-shape`；不代表 full platform production ready） |
| S04 | GPU 清单 / 占用 | NVIDIA device-plugin / DCGM | production-shaped gate passed（in-cluster Kubernetes API + cluster Service metrics；evidence：`live-evidence/sprint13-gpu-inventory-dcgm-live-evidence.json`；readiness：`sprint13-gpu-inventory-dcgm-readiness.md`；production guard：`validate-sprint13-b-track-production-shape`；不代表 full platform production ready） |
| S05 | 对象存储 bucket/upload/download | MinIO（S3 兼容，pre-signed URL） | production-shaped gate passed；B 轨已满足 `sprint13-production-shaped-gateway-profile.yaml` 中 S05 proof_items（result：`sprint13-objectstore-minio-live-result.md`；evidence：`live-evidence/sprint13-objectstore-minio-live-evidence.json`；gate：`validate-object-store-live-gate`） |
| S06 | 向量文档写入 | Milvus | production-shaped gate passed；B 轨已满足 `sprint13-production-shaped-gateway-profile.yaml` 中 S06 proof_items（result：`sprint13-vector-milvus-live-result.md`；evidence：`live-evidence/sprint13-vector-milvus-live-evidence.json`；readiness：`sprint13-vector-milvus-readiness.md`；gate：`validate-vector-store-live-gate`；历史 LIVE PENDING token 仅作门禁兼容语境） |
| S07 | 实例观测 logs/metrics/events | Prometheus + kubelet / K8s API | code+contract ready, LIVE PENDING；B 轨必须满足 `sprint13-production-shaped-gateway-profile.yaml` 中 S07 proof_items（见 `sprint13-instance-observability-prometheus-a-track.md`；readiness：`sprint13-instance-observability-prometheus-readiness.md`；gate：`validate-instance-observability-live-gate`） |

排序原则：S01–S04 已通过 production-shaped live gate，可作为 S05–S07 B 轨验收标准样板；S05–S07 的真实后端仍需 B 轨人工部署，且必须复用 S01-S04 已建立的 production-shaped proof_items 标准。
...
- S05/S06/S07 B 轨执行提示词（codex goal，达 production-shaped passed）：[`sprint13-s05-s07-b-track-prompts.md`](sprint13-s05-s07-b-track-prompts.md)
