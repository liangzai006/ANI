...
| S05 | 对象存储 bucket/upload/download | MinIO（S3 兼容，pre-signed URL） | code+contract ready, LIVE PENDING；B 轨必须满足 `sprint13-production-shaped-gateway-profile.yaml` 中 S05 proof_items（见 `sprint13-objectstore-minio-a-track.md`；readiness：`sprint13-objectstore-minio-readiness.md`；gate：`validate-object-store-live-gate`） |
| S06 | 向量文档写入 | Milvus | code+contract ready, LIVE PENDING；B 轨必须满足 `sprint13-production-shaped-gateway-profile.yaml` 中 S06 proof_items（见 `sprint13-vector-milvus-a-track.md`；readiness：`sprint13-vector-milvus-readiness.md`；gate：`validate-vector-store-live-gate`） |
