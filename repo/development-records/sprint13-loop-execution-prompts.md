...
| # | 切片 | 真实组件（已选） | status |
|---|---|---|---|
| S01 | 网络路由 | Kube-OVN v1.15.8 | real-provider evidence passed（见 `sprint13-netroute-kubeovn-live-result.md`；evidence：`live-evidence/sprint13-netroute-kubeovn-live-evidence.json`） |
| S02 | K8s workloads | vCluster / K8s v1.36.1 | real-provider evidence passed（见 `sprint13-k8s-workloads-vcluster-live-result.md`；evidence：`live-evidence/sprint13-k8s-workloads-vcluster-live-evidence.json`） |
| S03 | 卷快照 / mount-targets | Rook-Ceph（CSI snapshot / RBD / NFS） | real-provider evidence passed（见 `sprint13-storage-rook-ceph-live-result.md`；evidence：`live-evidence/sprint13-storage-rook-ceph-live-evidence.json`） |
| S04 | GPU 清单 / 占用 | NVIDIA device-plugin / DCGM | LIVE PENDING / BLOCKED on DCGM exporter evidence（A 轨见 `sprint13-gpu-inventory-dcgm-a-track.md`；B 轨只读盘点见 `sprint13-gpu-inventory-dcgm-live-result.md`；readiness：`sprint13-gpu-inventory-dcgm-readiness.md`；Gateway/Bootstrap `GPU_INVENTORY_PROVIDER=kubernetes_rest` 已补齐，device-plugin 真实可见但 DCGM exporter 未发现） |
| S05 | 对象存储 bucket/upload/download | MinIO（S3 兼容，pre-signed URL） | code+contract ready, LIVE PENDING（见 `sprint13-objectstore-minio-a-track.md`；readiness：`sprint13-objectstore-minio-readiness.md`；gate：`validate-object-store-live-gate`） |
| S06 | 向量文档写入 | Milvus | code+contract ready, LIVE PENDING（见 `sprint13-vector-milvus-a-track.md`；readiness：`sprint13-vector-milvus-readiness.md`；gate：`validate-vector-store-live-gate`） |
| S07 | 实例观测 logs/metrics/events | Prometheus + kubelet / K8s API | code+contract ready, LIVE PENDING（见 `sprint13-instance-observability-prometheus-a-track.md`；readiness：`sprint13-instance-observability-prometheus-readiness.md`；gate：`validate-instance-observability-live-gate`） |
