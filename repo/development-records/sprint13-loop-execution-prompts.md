...
| S04 | GPU 清单 / 占用 | NVIDIA device-plugin / DCGM | real-provider evidence passed（见 `sprint13-gpu-inventory-dcgm-live-result.md`；evidence：`live-evidence/sprint13-gpu-inventory-dcgm-live-evidence.json`；readiness：`sprint13-gpu-inventory-dcgm-readiness.md`） |
