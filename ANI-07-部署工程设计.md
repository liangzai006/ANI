...
> 最后更新：2026-05-19（对齐 v1.0.0 范围与 Karmada Phase 2 边界）

> 当前执行状态以 `ANI-DOCS-INDEX.md`、`ANI-06-开发计划.md` Section 零和 `repo/CURRENT-SPRINT.md` 为准。本文定义部署工程目标，不作为当前 Sprint 任务清单。
...
| 公有云托管 K8s（EKS/ACK/CCE）| Phase 2：Karmada 纳管 | 云原生 CNI | 云原生 StorageClass | 混合云/多云管理 |
...
Step 8   组件选择      基础版 | 完整版；Region/AZ 多集群模式为 Phase 2 预留
...
## 五、多 Region/AZ 架构（Karmada，Phase 2 预留）

> 本章是 Phase 2 架构预留，不是 v1.0.0 Phase 1 当前实现范围。
> v1.0.0 只要求单管理域内的安装、已有 K8s 接入，以及 ANI Core Sprint 5 的 vCluster 租户集群能力。
...
**v1.0.0 交付范围：** 不交付 Karmada 多集群；只保留接口与数据模型预留。
**Phase 2 交付范围：** Karmada 纳管、Region/AZ CRD、租户 AZ 放置 API、跨 AZ LB（全局 VIP）、跨 AZ NAT、MinIO Site Replication。
...
| Karmada 纳管 + Region/AZ CRD + Controller | Go | 3 周 | Phase 2 |
| 计量服务（TimescaleDB + 采集 Pipeline）| Go | 2 周 | M3-M4 |
| Patch 升级 Operator + BOSS 升级界面 | Go + TS | 2 周 | Phase 2 |
| BOSS 白牌化（API + 前端动态加载）| Go + TS | 1 周 | Phase 2 |
...
- ✅ 内部 CA + 纯 IP HTTPS 访问
- ✅ 计量基础版（用量记录，无账单）
- ✅ vCluster 租户 K8s 集群能力由 ANI Core Sprint 5 交付，Installer 负责部署所需底座
- ⏳ Patch 升级机制、BOSS 白牌化、Karmada 多集群进入 Phase 2
...
| Patch 升级 | Phase 2 测试项 | 不作为 v1.0.0 阻塞门禁 |
| 白牌化 | Phase 2 测试项 | 不作为 v1.0.0 阻塞门禁 |
| 计量 | 调用推理 10 次，查询计量 DB | Token 数记录准确 |
| AZ 放置 | Phase 2 测试项 | 不作为 v1.0.0 阻塞门禁 |
